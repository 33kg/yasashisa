...
function create_post_type() {
	register_post_type( 'items', /* post-type */
		array(
			'labels' => array(
				'name' => __( '商品一覧' ),
				'singular_name' => __( '商品一覧' )
			),
			'public' => true,
			'has_archive' => true,
			'menu_position' =>5,
	...
	register_taxonomy(
		'mediainfo_category', /* タクソノミーの名前 */
		'mediainfo', /* books投稿で設定する */
		array(
		'hierarchical' => true, /* 親子関係が必要なければ false */
		'update_count_callback' => '_update_post_term_count',
		'label' => 'メディア情報カテゴリー',
		'singular_label' => 'メディア情報カテゴリー',
		'public' => true,
		'show_ui' => true
		)
	);
	
	register_post_type( 'stock', /* post-type */
		array(
			'labels' => array(
				'name' => __( 'TOP在庫情報' ),
				'singular_name' => __( 'TOP在庫情報' )
			),
			'public' => true,
			'has_archive' => true,
			'menu_position' =>8,
	...
	register_post_type( 'syabonya-voice', /* post-type */
		array(
			'labels' => array(
				'name' => __( 'お客様の声(京都しゃぼんや)' ),
				'singular_name' => __( 'お客様の声(京都しゃぼんや)' )
			),
			'public' => true,
			'has_archive' => true,
			'menu_position' =>9,
...
function foo_modify_query_exclude_category( $query ) {
	// 管理画面以外 かつ メインクエリー かつ タクソノミーアーカイブのとき
	if ( ! is_admin() && $query->is_main_query() && $query->is_tax() )
		$query->set( 'posts_per_page', '200'); // 200件ずつ表示
...
　----- カスタム投稿タイプでタグを使う
*/
add_action('init', 'custom_post_tag');
